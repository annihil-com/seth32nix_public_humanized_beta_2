...
bool CHook::getSymbols(cgMod_t *mod, void *handle, void *mbase)
{
	...
	mod->CG_PlayVoiceChat = 				SYMBOL("CG_PlayVoiceChat");
	...
	mod->CG_AddPMItem = 					SYMBOL("CG_AddPMItem");
	...
	mod->CG_CenterPrint = 					SYMBOL("CG_CenterPrint");
	mod->CG_PriorityCenterPrint = 			SYMBOL("CG_PriorityCenterPrint");
	...
	Debug.Log("%s=0x%x","CG_PlayVoiceChat", mod->CG_PlayVoiceChat);
	...
	Debug.Log("%s=0x%x","CG_AddPMItem", mod->CG_AddPMItem);
	...
	Debug.Log("%s=0x%x","CG_CenterPrint", mod->CG_CenterPrint);
	Debug.Log("%s=0x%x","CG_PriorityCenterPrint", mod->CG_PriorityCenterPrint);
...
void CHook::hookCgame(bool state)
{
	...
	if (state && !hooked)
	{
		...
		CG_HOOK(CG_AddPMItem)

				if(eth32.cgMod->CG_AddPMItem2) {
					if (ETH32_DEBUG)
						orig_CG_AddPMItem2 = (CG_AddPMItem2_t)Detour.CreateDetour((void *)((uint32)eth32.cg.module+eth32.cgMod->CG_AddPMItem2), (void *)&eth32_CG_AddPMItem2);
				}

//		CG_HOOK(CG_AddPMItem2)
	...
	switch (command)
	{
		...
		case CG_GETSERVERCOMMAND:
		{
			...
			if (!strcmp(cmd, "gstats")){
				...
				// dont pass this one to cgame
				if (!eth32.cg.wantStats)
					return 0;
			}
			...
			if (eth32.cgMod->type==MOD_ETPRO && (!strcmp(cmd,"c") || !strcmp(cmd,"tc")) ) {
				...
				if (!strcmp(cmd,"c"))
					sprintf(buf, "%s^7: ^2%s", eth32.cg.players[cl].name, Syscall.CG_Argv(2));
				else
					sprintf(buf, "%s^7: ^5%s", eth32.cg.players[cl].name, Syscall.CG_Argv(2));
...
int CDECL eth32_CG_vmMain(int command, ...)
{
	...
	switch (command)
	{
		...
		{
			char *ui_cmd = strdup(Syscall.CG_Argv(0));
...
int CDECL eth32_UI_vmMain(int command, ...)
{
	...
	switch (command)
	{
		...
		{
			char *ui_cmd = strdup(Syscall.UI_Argv(0));
...
void CDECL eth32_LoadTGA( const char *name, byte **pic, int *width, int *height)
{
	...
	orig_LoadTGA(name, pic, width, height);
	  ...
	  if (eth32.settings.customSkies) {
		eth32.settings.pbScreenShot = 1;

	      //Fueldump custom sky
		if (!strcmp(name,"textures/skies/fueldump_clouds.tga"))
	{ 
		strcpy((char *)name, "gfx/misc/fuel_sky.tga");
		orig_LoadTGA(name, pic, width, height);
		
		// see if image failed to load (for xample wrong/bad pk3)
		if (!*pic){
			strcpy((char *)name, "textures/skies/fueldump_clouds.tga");
			orig_LoadTGA(name, pic, width, height);
		}
	}
    }
	...
	{ 
		strcpy((char *)name, "gfx/nullshader.tga");
		orig_LoadTGA(name, pic, width, height);
...
void CDECL eth32_etproAntiCheat(void *a, void *b, void *c, int checksum, void *e, char *orig_guid)
{
	...
	orig_etproAntiCheat(a, b, c, eth32.settings.etproOs ? etpro_linux : etpro_win32, e, eth32.settings.etproGuid);
}
...
char *eth32_Cvar_VariableString(const char *var_name)
{
	...
	if (eth32.pb.statusOK) {
		if (!strcmp(var_name, "cg_thirdperson") || !strcmp(var_name, "r_singleshader") || !strcmp(var_name, "cg_thirdpersonangle") || !strcmp(var_name, "cg_thirdpersonrange") || !strcmp(var_name, "r_shownormals")) {
			buffer[0] = '0';
			buffer[1] = '\0';
			return buffer;
		}
