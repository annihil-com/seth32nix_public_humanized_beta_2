...
void CTools::Init(int Game)
{
	...
	// load in namelist, guidlist and jaymac lists
	char file[256];
	...
	char guidfile[256];
	sprintf(guidfile, "%s/guids.txt", eth32.path);
	FILE *fp = fopen(guidfile, "r");
	nGuids = 0;
	if (fp) {
		char guid[256];
		while (fgets(guid, 256, fp)) {
			guidList = (char (*)[256]) realloc(guidList, (nGuids+1)*256);
			char *q = strchr(guid, '\n');
			if (q)
				*q = '\0';
			strcpy(guidList[nGuids], guid);
			nGuids++;
		}
	char macfile[256];
	sprintf(macfile, "%s/jaymacs.txt", eth32.path);
	FILE *fp = fopen(macfile, "r");
	nMacs = 0;
	if (fp) {
		char mac[256];
		while (fgets(mac, 256, fp)) {
			macList = (char (*)[256]) realloc(macList, (nMacs+1)*256);
			char *q = strchr(mac, '\n');
			if (q)
				*q = '\0';
			strcpy(macList[nMacs], mac);
			nMacs++;
		}
	char clguidfile[256];
	sprintf(clguidfile, "%s/clguids.txt", eth32.path);
	FILE *fp = fopen(clguidfile, "r");
	nClguids = 0;
	if (fp) {
		char clguid[256];
		while (fgets(clguid, 256, fp)) {
			clguidList = (char (*)[256]) realloc(clguidList, (nClguids+1)*256);
			char *q = strchr(clguid, '\n');
			if (q)
				*q = '\0';
			strcpy(clguidList[nClguids], clguid);
			nClguids++;
		}
	}
}
...
uint32 CTools::GetPrivateProfileString(char* lpAppName,char* lpKeyName,char* lpDefault,char* lpReturnedString,uint32 nSize,char* lpFileName)
{
	FILE* fp;
	bool found = false;
	int rightApp=0;
...
bool CTools::WritePrivateProfileString(char *lpAppName, char *lpKeyName, char *lpString, char *lpFileName)
{
	FILE* fp, *fp1;
	int n = -1;
	char line[1024];
...
void CTools::getOffsetFromINI( char *key, uint32 *offset )
{
	char value[64];
	int i;
...
void CTools::RegisterThread(uint32 tID, char *name)
{
	if (!this->nThreads)
...
void CTools::BufferConsoleCommand(const char *cmd)
{
#ifdef ETH32_DEBUG
	printf("CTools::BufferConsoleCommand: %s", cmd);
#endif
	if (!cmd || !(*cmd) || !eth32.cg.hooked)
		return;

	if (pthread_mutex_lock(cc_mutex)) FATAL_ERROR("could not lock mutex")

	if (numConsoleCmds < TOOLS_MAX_CONSOLE_CMDS) {
		strcpy(&consoleCmdBuffer[numConsoleCmds][0], cmd);
		numConsoleCmds++;
	}

	if (pthread_mutex_unlock(cc_mutex)) FATAL_ERROR("could not unlock mutex")
}

void CTools::PurgeConsoleCommands(void)
{
	if (pthread_mutex_lock(cc_mutex)) FATAL_ERROR("could not lock mutex")
		for (int i=0 ; i < numConsoleCmds ; i++)
			if (consoleCmdBuffer[i][0])
				Syscall.CG_SendConsoleCommand(consoleCmdBuffer[i]);
