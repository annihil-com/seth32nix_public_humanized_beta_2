...
//----(SA)	end
	...
	/* forty - Commenting these out prevents overbounces.
...
static void PM_Weapon( void ) {
	...
	if ( pm->ps->stats[STAT_HEALTH] <= 0 ) {
		if(!(pm->ps->pm_flags & PMF_LIMBO)) {
			PM_CoolWeapons();
			// Dens: to prevent player ending up with WP_NONE
			// when weapon switching while being killed
...
//		PM_CoolWeapons(); // Gordon: Arnout says this is how it's wanted ( bleugh ) no cooldown on weaps while using mg42, but need to update heat on mg42 itself
  ...
  if (pm->ps->weaponstate != weaponstate_last) {
	...
	if ( 
		...
		deltaTime = pm->cmd.serverTime - (pm->pmext->weapRecoilTime + 50);
...
//	}
	...
	switch(pm->ps->weapon) {
		...
		case WP_THOMPSON:
		case WP_STEN:
		// forty - Jaquboss medpack animation fix.
		// case WP_MEDKIT:
		case WP_PLIERS:
...
//			PM_ContinueWeaponAnim(weapattackanim);
	...
	// tjw: this causes all grenades to have 2.5s timer
	//      from the moment they leave your hand regardless
	//      of how long you held them as per fire_grenade()
	//      is this in the wrong spot, only for CGAME,
	//      or even necessary?
...
// tjw: for every ugly problem there exists an uglier solution.
...
//      to escape.
static void PM_EscapeSolid(void)
{
	trace_t trace;
	//vec3_t maxs;
	qboolean xgrowing = qtrue;
	qboolean ygrowing = qtrue;
	qboolean xshrink = qtrue;
	qboolean yshrink = qtrue;
	vec3_t maxs, mins;
	vec3_t maxOffsets, minOffsets;

	// tjw: if they're moving they are not stuck... yet
	if(pm->ps->velocity[0] || pm->ps->velocity[1] || pm->ps->velocity[2])
		return;

	//PM_TraceAll(&trace, pm->ps->origin, pm->ps->origin);
	pm->trace(&trace, pm->ps->origin,
		pm->mins, pm->maxs, pm->ps->origin,
		pm->ps->clientNum, pm->tracemask);
	if(!trace.startsolid) {
		return;
	}
	//Com_Printf("stuck in solid\n");
	VectorSet(maxs, 0, 0, pm->maxs[2]);
	VectorSet(mins, 0, 0, pm->mins[2]);
	pm->trace(&trace, pm->ps->origin,
				mins, maxs, pm->ps->origin,
				pm->ps->clientNum, pm->tracemask);
	if(trace.startsolid) {
		//Com_Printf("giving up, we're really wedged\n");
		return;
	}

	while (xgrowing || ygrowing || xshrink || yshrink) {
		if(pm->maxs[0] == maxs[0])
			xgrowing = qfalse;
		if(pm->maxs[1] == maxs[1])
			ygrowing = qfalse;
	...
	VectorSubtract(pm->maxs, maxs, maxOffsets);
	VectorSubtract(pm->mins, mins, minOffsets);
	//Com_Printf("max %s maxOffsets %s min %s minOffsets %s\n",
	//	vtos(maxs), vtos(maxOffsets),
	//	vtos(mins), vtos(minOffsets));

	pm->ps->origin[0] -= minOffsets[0] + maxOffsets[0];
	pm->ps->origin[1] -= minOffsets[1] + maxOffsets[1];
}
...
void PmoveSingle (pmove_t *pmove) {
	...
	if ( pm->ps->pm_type == PM_DEAD ) {
		...
		if( pm->ps->weapon == WP_SATCHEL_DET ) {
			if( !( pm->ps->ammoclip[ WP_SATCHEL_DET ] ) ) {
				...
				else if(CG_WeaponSelectable(WP_SILENCER))
					cg.weaponSelect = WP_SILENCER;
	...
	PM_EscapeSolid();
	//Com_Printf("maxs %s psmaxs %s mins %s psmins %s\n",
	//	vtos(pm->maxs), vtos(pm->ps->maxs),
	//	vtos(pm->mins), vtos(pm->ps->mins));
