...
#define ETH32_REQUEST_OFFSET	0x1337

__attribute__ ((visibility ("default")))
int eth32_clientdll_version()
{
	return ETH32_CLIENT_DLL_VERSION;
}
...
void eth32_get_offsets(void *p, void *b) 
{
	...
	cgMod_t *mod = (cgMod_t *)p;
	
	mod->automod = 1;
	strcpy(mod->name, "etmain");
	strcpy(mod->version, "2.60");
	mod->type = 0;
	mod->refdef = (uint32)&cg.refdef - base;
	mod->refdefViewAngles = (uint32)&cg.refdefViewAngles - base;
	mod->pmext = (uint32)&cg.pmext - base;
	
	int cc_total;
	uint32 cc_commands = (uint32)GetAddressOfCommands(&cc_total) - base;

	mod->commands = cc_commands;
	mod->numCommands = (uint32)cc_total;
	mod->predictedPS = (uint32)&cg.predictedPlayerState - base;
	mod->centBase = (uint32)&cg_entities - base;
	mod->centSize = sizeof(centity_t);
	mod->centCurrentStateOffset = offsetof(centity_t, currentState);
	mod->centLerpOrigin = offsetof(centity_t, lerpOrigin);
	mod->MOD_SUICIDE = MOD_SUICIDE;
	mod->ciBase = (uint32)&cgs.clientinfo - base;
	mod->ciSize = sizeof(clientInfo_t);
	mod->ciInfoValidOffset = offsetof(clientInfo_t, infoValid);
	mod->ciTeamOffset = offsetof(clientInfo_t, team);
	mod->ciNameOffset = offsetof(clientInfo_t, name);
	mod->ciHealthOffset = offsetof(clientInfo_t, health);
	mod->ciClassOffset = offsetof(clientInfo_t, cls);
	mod->CG_Player = (uint32)&CG_Player - base;
	mod->CG_AddPlayerWeapon = (uint32)&CG_AddPlayerWeapon - base;
	mod->CG_PositionRotatedEntityOnTag = (uint32)&CG_PositionRotatedEntityOnTag - base;
	mod->CG_SetLerpFrameAnimationRate = (uint32)&CG_SetLerpFrameAnimationRate - base;
	mod->CG_FinishWeaponChange = (uint32)&CG_FinishWeaponChange - base;
	mod->CG_PlayVoiceChat = (uint32)&CG_PlayVoiceChat - base;
	mod->CG_EntityEvent = (uint32)&CG_EntityEvent - base;
	mod->CG_Trace = (uint32)&CG_Trace - base;
	mod->CG_DamageFeedback = (uint32)&CG_DamageFeedback - base;
	mod->CG_WeaponFireRecoil = (uint32)&CG_WeaponFireRecoil - base;
	mod->CG_Respawn = (uint32)&CG_Respawn - base;
	mod->CG_CalculateReinfTime = (uint32)&CG_CalculateReinfTime - base;
	mod->CG_AllocLocalEntity = (uint32)&CG_AllocLocalEntity - base;
	mod->CG_AddPMItem = (uint32)&CG_AddPMItem - base;
	mod->CG_AddPMItem2 = 0;
	mod->CG_DrawPlayerStatusHead = (uint32)&CG_DrawPlayerStatusHead - base;
	mod->CG_DrawPlayerStats = (uint32)&CG_DrawPlayerStats - base;
	mod->CG_DrawPlayerStatus = (uint32)&CG_DrawPlayerStatus - base;
	mod->CG_CenterPrint = (uint32)&CG_CenterPrint - base;
	mod->CG_PriorityCenterPrint = (uint32)&CG_PriorityCenterPrint - base;
	mod->CG_ChargeTimesChanged = (uint32)&CG_ChargeTimesChanged - base;
	mod->BG_FindClipForWeapon = (uint32)&BG_FindClipForWeapon - base;
	mod->BG_FindAmmoForWeapon = (uint32)&BG_FindAmmoForWeapon - base;
	mod->CG_CalcViewValues = (uint32)&CG_CalcViewValues - base;	
}
