...
static void CG_ForceCvar() {
	...
	s = CG_Argv(1);
	Q_strncpyz(cvarName, s, sizeof(cvarName));
...
static void CG_DynoCounter() {
	...
	switch(type) {
		...
		case 0:
			...
			// find a free slot
			// Dens: just use MAXDYNAMITE here, not cg_maxTeamDynas. It won't do
		...
		// defused or exploded - remove it from the list.
		case 1:
		case 2:
...
static void CG_ParseScore( team_t team ) {
	...
	for(j = 0; j < numScores; j++) {
		...
		cg.scores[i].playerRating = cg.playerRating[i];
...
void CG_ParseEtpubinfo( void ) {
	...
	cgs.playerRating = atoi(Info_ValueForKey(info, "g_playerRating"));
	...
	if (etpub < ETPUB_VERSION(0, 8, 0)) {
		...
		Q_strncpyz(cgs.team_maxFlamers, "-1", sizeof(cgs.team_maxFlamers));
		Q_strncpyz(cgs.team_maxMG42s, "-1", sizeof(cgs.team_maxMG42s));
		Q_strncpyz(cgs.team_maxGrenLaunchers, "-1", sizeof(cgs.team_maxGrenLaunchers));
	} else {
		// Do our own checking
		Q_strncpyz(cgs.team_maxPanzers, Info_ValueForKey(info, "team_maxPanzers"), sizeof(cgs.team_maxPanzers));
		Q_strncpyz(cgs.team_maxMortars, Info_ValueForKey(info, "team_maxMortars"), sizeof(cgs.team_maxMortars));
		Q_strncpyz(cgs.team_maxFlamers, Info_ValueForKey(info, "team_maxFlamers"), sizeof(cgs.team_maxFlamers));
		Q_strncpyz(cgs.team_maxMG42s, Info_ValueForKey(info, "team_maxMG42s"), sizeof(cgs.team_maxMG42s));
	...
	if (etpub < ETPUB_VERSION(0, 8, 0)) {
		...
		Q_strncpyz(cgs.skill_medic, "20 50 90 140", sizeof(cgs.skill_medic));
		InitSkillLevelStructure(SK_FIRST_AID);
		Q_strncpyz(cgs.skill_engineer, "20 50 90 140", sizeof(cgs.skill_engineer));
		InitSkillLevelStructure(SK_EXPLOSIVES_AND_CONSTRUCTION);
		Q_strncpyz(cgs.skill_fieldops, "20 50 90 140", sizeof(cgs.skill_fieldops));
		InitSkillLevelStructure(SK_SIGNALS);
		Q_strncpyz(cgs.skill_covertops, "20 50 90 140", sizeof(cgs.skill_covertops));
		InitSkillLevelStructure(SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS);
		Q_strncpyz(cgs.skill_battlesense, "20 50 90 140", sizeof(cgs.skill_battlesense));
		InitSkillLevelStructure(SK_BATTLE_SENSE);
		Q_strncpyz(cgs.skill_lightweapons, "20 50 90 140", sizeof(cgs.skill_lightweapons));
		InitSkillLevelStructure(SK_LIGHT_WEAPONS);
	} else {
		// Do our own checking
		Q_strncpyz(cgs.skill_soldier, Info_ValueForKey(info, "skill_soldier"), sizeof(cgs.skill_soldier));
		InitSkillLevelStructure(SK_HEAVY_WEAPONS);
		Q_strncpyz(cgs.skill_medic, Info_ValueForKey(info, "skill_medic"), sizeof(cgs.skill_medic));
		InitSkillLevelStructure(SK_FIRST_AID);
		Q_strncpyz(cgs.skill_engineer, Info_ValueForKey(info, "skill_engineer"), sizeof(cgs.skill_engineer));
		InitSkillLevelStructure(SK_EXPLOSIVES_AND_CONSTRUCTION);
		Q_strncpyz(cgs.skill_fieldops, Info_ValueForKey(info, "skill_fieldops"), sizeof(cgs.skill_fieldops));
		InitSkillLevelStructure(SK_SIGNALS);
		Q_strncpyz(cgs.skill_covertops, Info_ValueForKey(info, "skill_covertops"), sizeof(cgs.skill_covertops));
		InitSkillLevelStructure(SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS);
		Q_strncpyz(cgs.skill_battlesense, Info_ValueForKey(info, "skill_battlesense"), sizeof(cgs.skill_battlesense));
		InitSkillLevelStructure(SK_BATTLE_SENSE);
		Q_strncpyz(cgs.skill_lightweapons, Info_ValueForKey(info, "skill_lightweapons"), sizeof(cgs.skill_lightweapons));
		InitSkillLevelStructure(SK_LIGHT_WEAPONS);
	}
}
...
=======================
*/
/*	kw: this never gets drawn so it's a waste
...
void CG_parseWeaponStats_cmd(void (txt_dump)(char *))
{
	...
	// CHRUKER: b037 - Negative XP wouldn't show up on the playerstat screen
	int xp = 0;
